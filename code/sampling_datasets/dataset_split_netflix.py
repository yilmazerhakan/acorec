...
# Sampling parameters
# for users at least 20 items ranked and maximum 500 items ranked
minu = 20
maxu = 500
# for items at least ranked by 20 users and ranked by maximum 500 users
mini = 20
maxi = 500
min_rating = 4

if os.getenv('USER')=="hyilmazer":
    sys.stdout = open("/truba_scratch/hyilmazer/benchmarks/ipana/{}_{}_{}_{}_{}_netflix.txt".format(minu,maxu,mini,maxi,min_rating), "w", buffering=1)
def print(text):
    builtins.print(text)
    os.fsync(sys.stdout)
...
tp = pd.read_table('/truba/home/hyilmazer/.cornac/netflix/data.csv', header=None, sep=',', usecols=[0, 1, 2], names=['uid', 'sid', 'rating'])
# unique uids
...
def filter_triplets(df, minu,maxu,mini,maxi,min_rating):
    ...
    df = df[df['sid'].isin(item_counts[item_counts >= mini].index)]
    df = df[df['sid'].isin(item_counts[item_counts <= maxi].index)]
    
    user_counts = df['uid'].value_counts()
    df = df[df['uid'].isin(user_counts[user_counts >= minu].index)]
    df = df[df['uid'].isin(user_counts[user_counts <= maxu].index)]
