...
class Recommender:
    ...
    def rank(self, user_idx, item_indices=None):
        ...
        ----------
        user_idx: int, required
            The index of the user for whom to perform item ranking.
        ...
        # !!! UPDATE user clicked items as zero
        # we need to evaluate unclicked items
        pos_items = self.train_set.user_data.get(user_idx)[0]
        known_item_scores = known_item_scores.flatten()
        known_item_scores[pos_items] = 0.0
        ...
        # !!! UPDATE
        # Add top-10 items to recommended items
        self.recommended_item_set.append(item_rank[:10])
