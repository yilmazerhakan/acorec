...
def ranking_eval(
    ...
    for user_idx in tqdm(
        ...
        u_gt_pos = np.zeros(test_set.num_items, dtype=np.int_)
        ...
        u_gt_neg = np.ones(test_set.num_items, dtype=np.int_)
    ...
    for i, mt in enumerate(metrics):
        ...
        # new codes
        # avg_results.append(sum(user_results[i].values()) / np.sum(gt_mat))
...
class BaseMethod:
    ...
    def _build_datasets(self, train_data, test_data, val_data=None):
        ...
        if hasattr(self.train_set, 'uid_map'):
            self.global_uid_map=self.train_set.uid_map
        if hasattr(self.train_set, 'iid_map'):
            self.global_iid_map=self.train_set.iid_map
    ...
    def evaluate(self, model, metrics, user_based, show_validation=True, aco_eval_repeat=1):
        ...
        if model.name.startswith("AcoRec"):
            ...
            for i in range(eval_repeat):
                ...
                ###########
                if self.verbose:
                    print("\n Eval:{},  [{}] Training started!".format(i+1, model.name))
                ...
                ##############
                if self.verbose:
                    print("\nEval: {} of [{}] Evaluation started!".format(i+1, model.name))
                ...
                if show_validation and self.val_set is not None:
                    if val_result is None:
                        eval_val_result = val_result 
                    else:
                        for key in val_result.metric_avg_results.keys():
                            eval_val_result.metric_avg_results[key] = (val_result.metric_avg_results.get(key, 0) 
                                                                    + eval_val_result.metric_avg_results.get(key, 0))

            if eval_repeat>1:
                for key in eval_test_result.metric_avg_results.keys():
                    eval_test_result.metric_avg_results[key] = float(eval_test_result.metric_avg_results.get(key, 0) / eval_repeat)
                test_result = eval_test_result
                if show_validation and self.val_set is not None:
                    for key in eval_val_result.metric_avg_results.keys():
                        eval_val_result.metric_avg_results[key] = float(eval_val_result.metric_avg_results.get(key, 0) / eval_repeat)
                    val_result = eval_val_result  
        else:
            ###########
            # FITTING #
            ###########
            ...
            if hasattr(model, 'best_params'):
                if len(model.eval_method)>1:
                    old_model = model.model.clone(model.best_params)
                    eval_test_result = None
                    eval_val_result = None
                    for evali in model.eval_method:
                        tr_set = evali.train_set
                        vl_set = evali.val_set
                        start = time.time()
                        emodel = old_model.fit(tr_set, vl_set)
                        train_time = time.time() - start
                        if self.verbose:
                            print("\nEval: [{}] Evaluation started!".format(model.name))

                        start = time.time()
                        test_result = self._eval(
                            model=emodel,
                            test_set=evali.test_set,
                            val_set=None,
                            user_based=user_based,
                        )
                        test_time = time.time() - start
                    ...
                    for key in eval_test_result.metric_avg_results.keys():
                        eval_test_result.metric_avg_results[key] = float(eval_test_result.metric_avg_results.get(key, 0) / len(model.eval_method))
                    test_result = eval_test_result
                    val_result = None
            else:
                if self.verbose:
    ...
    def evaluatecv(self, model, metrics, user_based, show_validation=True):
        """Evaluate given models according to given metrics
        ...
        if model.name.startswith("AcoRec"):
            ...
            for i in range(eval_repeat):
                ...
                ###########
                if self.verbose:
                    print("\n Eval: {} of [{}] Training started!".format(i+1, model.name))
            ...
            ###########
            if self.verbose:
                print("\n[{}] CV Training started!".format(model.name))
